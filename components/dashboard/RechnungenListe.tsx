...
export default function RechnungenListe({ rechnungen, updRe, delRe, nav, dupRe, firma, onEdit, initialSearch = "", showT, nxtNr: _nxtNr, plan = "free", konvertierAngebot }: { rechnungen: Rechnung[]; updRe: (id: string, up: Partial<Rechnung>) => void; delRe: (id: string) => void; nav: (pg: string) => void; dupRe: (r: Rechnung) => Promise<void>; firma: Firma | null; onEdit: (r: Rechnung) => void; initialSearch?: string; showT: (msg: string) => void; nxtNr: () => string; plan?: string; konvertierAngebot: (id: string, datum: string) => Promise<void> }) {
  ...
  const sbtn = "flex items-center gap-1.5 px-3 py-2 max-md:px-2 bg-white/[0.05] text-slate-300 border border-white/[0.08] rounded-lg text-[11px] cursor-pointer whitespace-nowrap hover:bg-white/[0.08] transition-all font-medium";
  const sbtnG = "flex items-center gap-1.5 px-3 py-2 max-md:px-2 bg-success-500/10 text-success-400 border border-success-500/20 rounded-lg text-[11px] cursor-pointer whitespace-nowrap font-medium hover:bg-success-500/15 transition-all";
  const dbtn = "flex items-center px-2.5 py-2 max-md:px-2 bg-danger-500/10 text-danger-400 border border-danger-500/20 rounded-lg text-[11px] cursor-pointer font-medium hover:bg-danger-500/15 transition-all";
