...
export default function KundenListe({ kunden, rechnungen, updKu, delKu }: { kunden: Kunde[]; rechnungen: Rechnung[]; updKu: (id: string, up: Partial<Kunde>) => void; delKu: (id: string) => void }) {
  ...
  const { navNewDocForKunde } = useNavigation();
    ...
    <div className="p-6 px-7 max-md:p-4 animate-fade-in">
      ...
      {f.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-12 text-center">
          ...
          <h2 className="text-lg font-bold">Keine Kunden</h2>
        </div>
      ) : (
        <div className="bg-[#0a0a1a]/80 border border-white/[0.06] rounded-2xl overflow-hidden">
          {f.map((k, i) => {
            ...
            return (
              <div
                key={k.id}
                className={`group flex items-center gap-3 px-4 py-3.5 hover:bg-white/[0.03] transition-all duration-150 ${i < f.length - 1 ? "border-b border-white/[0.05]" : ""}`}
              >
                {/* Avatar */}
                ...
                <div className="flex items-center gap-1.5 shrink-0">
                  <button
                    ...
                    onClick={() => navNewDocForKunde(k.id)}
